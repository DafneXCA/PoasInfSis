...
<div class="body-content">
    <div class="titulo"><strong>Objetivos de específicos</strong> <button  data-bs-toggle="modal" data-bs-target="#staticBackdrop" class="button-añadir" id="añadir">Añadir</button></div>
    <div class="content-div">
        <div class="content-table">
            <table>
                ...
                <tbody>
                    @foreach ($objGestion->objetivosE as $objetivoE)
                        <tr >
                            <td>{{$objetivoE->objetivo}}</td>
                            <td>{{$objetivoE->indicador}}</td>
                            @auth
                                
                            @endauth
                            <td>
                                <a href="{{route('operacionesProyectos',['id'=>$objetivoE->id])}}"><i class="bi bi-file-earmark-text" title="operaciones o proyectos"></i></a>
                                <a onclick="editar({{$objetivoE->id}},'{{$objetivoE->objetivo}}','{{$objetivoE->indicador}}')" data-bs-toggle="modal" data-bs-target="#editar" id="edit"><i class="bi bi-pen" title="editar"></i></a>
                                <a onclick="CompletarId({{$objetivoE->id}})" data-bs-toggle="modal" data-bs-target="#eliminar"><i class="bi bi-trash" title="eliminar"></i></a>
                            </td>
                        </tr>
                    @endforeach
...
<div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false"  aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header ">
          <h1 class="modal-title fs-5 " ><strong>Agregar Objetivo Especifico</strong></h1>
          <button class="btn-close" data-bs-dismiss="modal" ></button>
        </div>
        <div class="modal-body">
            <form action="{{route('objetivosEspecificos-store')}}" method="POST" id="form">
                @csrf

                <input type="hidden" name="obj_gestion" value="{{$objGestion->id}}">
  ...
  <div class="modal fade" id="editar" data-bs-backdrop="static" data-bs-keyboard="false"  aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header ">
          <h1 class="modal-title fs-5 " ><strong>Editar Objetivo Especifico</strong></h1>
          <button class="btn-close" data-bs-dismiss="modal" ></button>
        </div>
        <div class="modal-body">
            <form action="{{route('objetivosEspecificos-update')}}" method="POST" id="formEdit">
                @csrf
...
<!----modal eliminar---->
  ...
  <div class="modal fade" id="eliminar" data-bs-backdrop="static" data-bs-keyboard="false"  aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header ">
          <h1 class="modal-title fs-5 " ><strong>Eliminar Objetivo Especifico</strong></h1>
          <button class="btn-close" data-bs-dismiss="modal" ></button>
        </div>
        <div class="modal-body">
            ¿Estas seguro de eliminar el objetivo especifico de la gestión?
        </div>
        <div class="modal-footer">
  ...
  <script>
    ...
    //eliminar
     function CompletarId(id){
        ...
        var route="{{route('objetivosEspecificos-destroy',['id'=>""])}}";
        route=route+"/"+id;
        eliminar.href=route;
    }
  </script>
